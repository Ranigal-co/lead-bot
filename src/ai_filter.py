...
async def verify(text: str) -> bool:
    ...
    prompt = (
        ...
        "Твоя задача — найти реальных клиентов, которые ищут фотографа или видеографа (свадьба, фотосессия, съёмка).\n"
        "Если пост написан САМИМ фотографом или видеографом, предлагающим свои услуги (портфолио, цены, приглашение на съёмку) — ответь НЕТ.\n"
        "Если пост — это заказ от клиента (поиск специалиста) — ответь ДА.\n"
        "Если это реклама, кастинг, обучение, вакансия НЕ для фотографа — ответь НЕТ.\n\n"
        f"Текст поста:\n{text[:1500]}"
    )
